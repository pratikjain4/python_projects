...
def retrieve_html_content():
    ...
    try:
        ...
        req = Request(page_url, headers=headers, method="GET")
        page_contents = urlopen(req).read()
...
def check_and_download_file(exe_link):
    ...
    if file_exists:
        ctypes.windll.user32.MessageBoxW(
            0, "Everything is up to date!", "Everything", 0
        )
        subprocess.Popen("explorer /select," + "downloaded_files\\" + filename)
    else:
        ctypes.windll.user32.MessageBoxW(
            0, "New version of Everything found!  Downloading....!", "Everything", 0
        )
